...
export function buildHistoricalMarketStackResearch(records = [], options = {}) {
  ...
  const scalar = buildStackResearch(input, options, buildHistoricalMarketFactorPrequentialStackPredictions(input, options), { format: 'investor-control-historical-market-stacked-ensemble-research', contract: HISTORICAL_MARKET_STACK_RESEARCH_CONTRACT, modelVariant: 'SCALAR_MARKET_FACTOR', features: ['PATTERN_LOGIT', 'HISTORICAL_MARKET_FACTOR_SCORE'], representation: 'CANONICAL_MOMENTUM_RISK_WEIGHTED_SCALAR', readyStatus: 'HISTORICAL_MARKET_STACK_PREDICTIVE_READY_GROUPS_EXIST', notReadyStatus: 'HISTORICAL_MARKET_STACK_PREDICTIVE_NOT_READY' });
  return {
    ...scalar,
    domainSeparatedCandidate: buildHistoricalMarketDomainStackResearch(input, options),
    priorShrunkCandidate: buildHistoricalMarketPriorShrunkStackResearch(input, options),
  };
}
