import {
  HISTORICAL_MARKET_DOMAIN_STACK_RESEARCH_CONTRACT,
  HISTORICAL_MARKET_STACK_RESEARCH_VERSION,
} from './forecast-historical-market-stacked-ensemble-research.js';

export const HISTORICAL_MARKET_DOMAIN_STACK_CANDIDATE_SAFETY_CONTRACT = 'HISTORICAL_MARKET_DOMAIN_STACK_CANDIDATE_SAFETY_V1';

function assert(condition, message) {
  if (!condition) throw new Error(`Historical market domain stack candidate safety: ${message}`);
}
...
export function verifyHistoricalMarketDomainStackCandidate(candidate = {}, options = {}) {
  assert(candidate && typeof candidate === 'object', 'candidate missing');
  assert(candidate.format === 'investor-control-historical-market-domain-stacked-ensemble-research', 'format invalid');
  ...
  assert(candidate.contract === HISTORICAL_MARKET_DOMAIN_STACK_RESEARCH_CONTRACT, 'contract invalid');
  assert(candidate.modelVariant === 'DOMAIN_SEPARATED_MARKET_FACTOR', 'model variant invalid');
  ...
  assert(candidate?.methodology?.model === 'PREQUENTIAL_L2_LOGISTIC_STACK', 'model invalid');
  assert(JSON.stringify(candidate?.methodology?.features) === JSON.stringify(['PATTERN_LOGIT', 'HISTORICAL_MOMENTUM_SCORE', 'HISTORICAL_RISK_SCORE']), 'features invalid');
  assert(candidate?.methodology?.representation === 'MOMENTUM_AND_RISK_PRESERVED_AS_SEPARATE_PREQUENTIAL_FEATURES', 'representation invalid');
  ...
  candidate.groups.forEach((group, index) => {
    assert(group?.modelVariant === 'DOMAIN_SEPARATED_MARKET_FACTOR', `group ${index} model variant invalid`);
  ...
  return {
    status: 'VERIFIED',
    contract: HISTORICAL_MARKET_DOMAIN_STACK_CANDIDATE_SAFETY_CONTRACT,
