...
test('Athens extractor searches the full document and accepts metrics only from the correct accounting statement', () => {
  const fillers = Array.from({ length: 8 }, (_, index) => `Management commentary page ${index + 1}\nNo audited statement table on this page.`);
  const pages = [
