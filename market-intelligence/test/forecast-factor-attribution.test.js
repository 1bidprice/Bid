...
test('feature-vector versions are never pooled',()=>{const s=buildForecastFactorAttributionStatus({records:[...Array.from({length:80},(_,i)=>rec(i,{vectorVersion:'fv-v1'})),...Array.from({length:80},(_,i)=>rec(i+80,{vectorVersion:'fv-v2'}))]});assert.equal(s.groupCount,2);});
test('strong domain ordering is supported',()=>{const m=buildForecastFactorAttributionStatus({records:Array.from({length:180},(_,i)=>rec(i))}).groups[0].domains.find(d=>d.domain==='MOMENTUM');assert.equal(m.status,'PREDICTIVE_DIRECTION_SUPPORTED');assert.ok(m.rocAuc>.95);assert.equal(m.automaticWeightAdjustmentEnabled,false);});
test('inverted domain is surfaced without auto reweight',()=>{const s=buildForecastFactorAttributionStatus({records:Array.from({length:220},(_,i)=>rec(i,{invert:true}))});const m=s.groups[0].domains.find(d=>d.domain==='MOMENTUM');assert.equal(m.status,'INVERTED_OR_NONPREDICTIVE');assert.equal(m.manualWeightReviewCandidate,true);assert.equal(s.automaticWeightAdjustmentEnabled,false);});
test('OPEN snapshots add coverage but cannot leak outcomes',()=>{const s=buildForecastFactorAttributionStatus({records:[...Array.from({length:90},(_,i)=>rec(i)),...Array.from({length:100},(_,i)=>rec(i+90,{open:true}))]});const m=s.groups[0].domains.find(d=>d.domain==='MOMENTUM');assert.equal(m.lineageCoverageCount,190);assert.equal(m.maturedSampleSize,90);assert.equal(m.positiveCount+m.negativeCount,90);assert.equal(m.status,'INSUFFICIENT_OOS_HISTORY');assert.equal(s.forecastMayInfluenceFinalAction,false);});
test('null and string outcomes never coerce into matured attribution samples',()=>{const base=Array.from({length:90},(_,i)=>rec(i));const nullOutcome={...rec(90),positiveOutcome:null};const stringZero={...rec(91),positiveOutcome:'0'};const stringOne={...rec(92),positiveOutcome:'1'};const s=buildForecastFactorAttributionStatus({records:[...base,nullOutcome,stringZero,stringOne]});const m=s.groups[0].domains.find(d=>d.domain==='MOMENTUM');assert.equal(m.lineageCoverageCount,93);assert.equal(m.maturedSampleSize,90);assert.equal(m.positiveCount+m.negativeCount,90);});
