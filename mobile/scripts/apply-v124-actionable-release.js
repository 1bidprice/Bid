...
function replaceRegexRequired(content, regex, replacement, label) {
  if (!regex.test(content)) throw new Error(`v1.2.3 canonical finalizer failed: missing ${label}`);
  return content.replace(regex, replacement);
}

function writeCanonicalDecisionCard() {
...
function patchPortfolioResponsiveStyles() {
  let source = read('PortfolioApp.js');
  source = replaceRegexRequired(source, /const VERSION = '1\.2\.2';/, "const VERSION = '1.2.3';", 'Portfolio v1.2.2 identity');
  source = replaceRegexRequired(
    source,
    /rowTop:\s*\{[^}]*\}/,
    "rowTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap' }, positionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap' }",
    'rowTop style',
  );
  source = replaceRegexRequired(
    source,
    /grow:\s*\{[^}]*\}/,
    "grow: { flexGrow: 1, flexShrink: 1, flexBasis: 180, minWidth: 180 }",
    'grow style',
  );
  source = replaceRegexRequired(
    source,
    /badge:\s*\{[^}]*\}/,
    "badge: { backgroundColor: '#edf4ff', paddingHorizontal: 13, paddingVertical: 9, borderRadius: 18, maxWidth: '100%', flexShrink: 1 }",
    'quote badge style',
  );
  source = replaceRegexRequired(
    source,
    /badgeText:\s*\{[^}]*\}/,
    "badgeText: { color: '#0B66FF', fontWeight: '900', fontSize: 13, textAlign: 'center', flexShrink: 1 }",
    'quote badge text style',
  );
  source = replaceRegexRequired(
    source,
    /reviewLine:\s*\{[^}]*\}/,
    "reviewLine: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap', borderBottomWidth: 1, borderBottomColor: '#e6ecf3', paddingVertical: 12 }",
    'settings review row',
  );
  source = replaceRegexRequired(
    source,
    /statusStrong:\s*\{[^}]*\}/,
    "statusStrong: { color: '#16345f', fontWeight: '900', textAlign: 'right', flexShrink: 1, maxWidth: '65%' }",
    'settings status text',
  );
  write('PortfolioApp.js', source);

  let decision = read('DecisionOverlay.js');
  decision = replaceRegexRequired(decision, /const VERSION = '1\.2\.2';/, "const VERSION = '1.2.3';", 'Decision v1.2.2 identity');
...
writeCanonicalDecisionCard();
patchPortfolioResponsiveStyles();
patchVersionFiles();
...
for (const invariant of ['ΠΛΑΝΟ ΤΩΡΑ — ΠΕΡΙΟΡΙΣΜΕΝΑ ΔΕΔΟΜΕΝΑ', 'controlledPlan', 'Δεν αποτελεί τελικό BUY/SELL σήμα']) {
  if (!finalCard.includes(invariant)) throw new Error(`v1.2.3 final-card verification failed: ${invariant}`);
}
for (const invariant of ["const VERSION = '1.2.3';", "flexWrap: 'wrap'", "maxWidth: '65%'", 'positionHeader']) {
  if (!portfolio.includes(invariant)) throw new Error(`v1.2.3 portfolio verification failed: ${invariant}`);
}
if (!decision.includes("const VERSION = '1.2.3';")) throw new Error('v1.2.3 DecisionOverlay version mismatch');
...
if (pkg.version !== '1.2.3') throw new Error('v1.2.3 package identity mismatch');
console.log('Investor Control v1.2.3 canonical actionable release finalizer applied.');
