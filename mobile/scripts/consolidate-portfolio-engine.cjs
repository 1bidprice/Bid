...
if (!portfolio.includes("const liveUsProviderSymbolsKey = liveUsProviderSymbols.join('|');")) throw new Error('stable live symbol dependency key missing');
if (!portfolio.includes('[applyQuotes, liveUsProviderSymbolsKey, loading, token]')) throw new Error('websocket effect must not reconnect on every price render');
...
console.log('Portfolio consolidation PASS: UI accounting removed, generic/stable US live subscription wired, market-data owns quote classification, verifier follows canonical engine.');
