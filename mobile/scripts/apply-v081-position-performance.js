...
function patchPortfolio() {
  ...
  source = replaceRequired(
    ...
    'position-lots import and version',
  );
  ...
  const oldCard = `function PositionCard({ item, compact, expanded, onToggle, onAlert }) {
  ...
  const newCard = `function PositionLotRow({ lot, currency, stale }) {
  ...
  const date = lot.date ? new Date(\\`\\${lot.date}T12:00:00\\`).toLocaleDateString('el-GR') : '—';
...
function PositionCard({ item, compact, expanded, onToggle, onAlert }) {
  ...
  if (!source.includes('function PositionLotRow(')) {
    source = replaceRequired(source, oldCard, newCard, 'PositionCard implementation');
  }
