...
if (!store.includes('const LEGACY_FEED_VERSION = 1;')) {
  if (store.includes('const FEED_VERSION = 2;\nconst SUPPORTED_FEED_VERSIONS = new Set([1, 2]);')) {
    store = store.replace(
      'const FEED_VERSION = 2;\nconst SUPPORTED_FEED_VERSIONS = new Set([1, 2]);',
      'const LEGACY_FEED_VERSION = 1;\nconst FEED_VERSION = 2;\nconst SUPPORTED_FEED_VERSIONS = new Set([LEGACY_FEED_VERSION, FEED_VERSION]);',
    );
  } else if (store.includes('const FEED_VERSION = 1;')) {
    store = store.replace(
      'const FEED_VERSION = 1;',
      'const LEGACY_FEED_VERSION = 1;\nconst FEED_VERSION = 2;\nconst SUPPORTED_FEED_VERSIONS = new Set([LEGACY_FEED_VERSION, FEED_VERSION]);',
    );
  } else {
    throw new Error('Investor Control v1.7.0 mobile patch failed: missing compatible feed version contract');
  }
}

if (!store.includes('SUPPORTED_FEED_VERSIONS.has(Number(payload?.version))')) {
  store = replaceRequired(
    store,
    'Number(payload?.version) !== FEED_VERSION',
    '!SUPPORTED_FEED_VERSIONS.has(Number(payload?.version))',
...
if (!store.includes('const opportunityPurchaseDecisions = safeArray(payload.opportunityPurchaseDecisions)')) {
  store = replaceRequired(
    store,
    `  return {\n    format: FEED_FORMAT,`,
    ...
    'normalized feed return anchor',
  );
}

if (!store.includes('opportunityCandidateCount: opportunityPurchaseDecisions.length')) {
  store = replaceRequired(
    store,
...
if (!store.includes('    opportunityPurchaseDecisions,')) {
  store = replaceRequired(
    store,
    '    discoveryRadar,',
...
if (!store.includes('    opportunityAssistantContext: opportunityPurchaseDecisions.map')) {
  store = replaceRequired(
    store,
...
if (!opportunities.includes('function OpportunityPurchaseCard({ item })')) {
...
if (!opportunities.includes('feed.confirmedBuyOpportunities || []')) {
  opportunities = replaceRequired(
    opportunities,
...
if (!opportunities.includes('!feed.opportunityPurchaseDecisions?.length')) {
  opportunities = replaceRequired(
    opportunities,
    '!feed.published?.length && !feed.reviewReady?.length && !feed.research?.length ?',
    '!feed.published?.length && !feed.reviewReady?.length && !feed.research?.length && !feed.opportunityPurchaseDecisions?.length ?',
